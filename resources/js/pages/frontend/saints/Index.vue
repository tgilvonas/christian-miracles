...
const personHref = (person: any) => {
    const slug = person?.slug ?? person?.id;
    if (!slug) {
        return '#';
    }

    return route(currentLocale.value.startsWith('lt') ? 'saints.show_lt' : 'saints.show_en', { slug });
};
...
watch([search, selectedLocation], () => {
    ...
    fetchTimer = window.setTimeout(() => fetchPersons(), 350);
});
</script>
        ...
        <div class="flex w-full items-center justify-center">
            <main class="w-full max-w-[1800px] mx-auto">
                ...
                <section class="grid gap-2">
                    ...
                    <div v-else-if="!persons.length">No results found.</div>
                    <Link
                        v-for="p in persons"
                        :key="p.id"
                        :href="personHref(p)"
                    ...
                    >
                        <article class="flex items-center gap-4">
                            <img v-if="p.intro_image_url" :src="p.intro_image_url" alt="" class="max-h-[10rem] max-w-[10rem] object-cover rounded bg-gray-100 dark:bg-gray-800" />
                            <div>
                                <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">{{ p.title ?? p.name }}</h2>
                                <div class="text-sm text-gray-700 dark:text-gray-300">{{ joinNames(p.locations) }}</div>
                            </div>
                        </article>
                    </Link>
                </section>
